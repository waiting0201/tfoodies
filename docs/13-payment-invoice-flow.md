...
> | 作廢 | `invoice_invalid`（**非** `invoice/void`）| `1.0` | **`RespondType=String`**（非 JSON）；只帶 `InvoiceNumber`（**非** `InvoiceNo`）＋ `InvalidReason`（同舊系統） |
...
>
> 🐞 **作廢改用 `RespondType=String` 對齊舊系統（2026-07-22 修）**：新系統作廢原用 `RespondType=JSON`，實測此 ezPay 帳號在 JSON 模式下對 `invoice_invalid` 套用「**開立(issue)等級**」的嚴格欄位驗證，逐一索取 `MerchantOrderNo→BuyerName→Category…`（作廢照理不需要這些）。改回舊系統 `AjaxController/CancelInv` 多年可用的請求：**`RespondType=String` ＋ 只帶 `InvoiceNumber`＋`InvalidReason`**（外加 `Version=1.0`/`TimeStamp`）即免。因 String 模式回應為 query-string（非 JSON），`EzPayInvoiceService.CallAsync` 加 `jsonResponse:false` 分支（`ParseQueryString` 讀 `Status`/`Message`）；`VoidAsync` 簽章回歸 `(invoiceNumber, reason)`。**折讓（`allowance_issue`）仍走 JSON**，其 `MerchantOrderNo` 須為 `Orders.ordercode`（非發票號）—`InvoiceAdminController` join `Orders` 取得，此為獨立既有 bug 一併修正。**須重新部署 tfoodies-api 才生效。**
>
