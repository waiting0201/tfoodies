...
> | 作廢 | `invoice_invalid`（**非** `invoice/void`）| `1.0` | 請求帶 `InvoiceNumber`（**非** `InvoiceNo`）**＋ `MerchantOrderNo`（= 訂單編號）＋ `BuyerName`（B2B 再帶 `BuyerUBN`）** |
...
>
> 🐞 **作廢須帶 `MerchantOrderNo` ＋ `BuyerName`（＋B2B `BuyerUBN`）（2026-07-22 修）**：此 ezPay 帳號的 `invoice_invalid` 驗證比手冊嚴，除 `InvoiceNumber` 外，**須逐一帶開立當時的 `MerchantOrderNo`（= `Orders.ordercode`）、`BuyerName`（B2B=公司抬頭、其餘=會員姓名），B2B 再帶 `BuyerUBN`（全大寫，統編）**，缺一即逐一回「**資料不齊全MerchantOrderNo / BuyerName**」。修正：`IInvoiceService.VoidAsync(invoiceNumber, merchantOrderNo, buyerName, buyerUbn?, reason)`，兩呼叫端（訂單詳情頁 `PaymentCompletionService.VoidInvoiceAsync`、發票管理 `InvoiceAdminController`）皆 join `Orders`/`Members` 帶入訂單編號與買受人（與開立同邏輯）。折讓端另修既有 bug：原誤把**發票號**當 `MerchantOrderNo` 送，改用 `ordercode`。⚠️ 舊系統 `AjaxController/CancelInv` 當年只帶 `InvoiceNumber`＋`InvalidReason` 即可作廢，ezPay 現已收緊。**須重新部署 tfoodies-api 才生效。**
>
