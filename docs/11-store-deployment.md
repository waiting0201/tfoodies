...
> ✅ **切換正式網域（已完成，2026-07-01）**：`storeCustomDomains` 清單已填入 4 個正式網域（主網域
> 排第 1 項）＋保留過渡網域 `tfoodies-store.4webdemo.com` 於清單最後一項（尚可用，未來確認無用途後可拿掉）。
> `siteUrl` 與清單第 1 項一致，均為 `https://www.tfoodies.com`。
>
> **4 個 hostname 全部直接服務（不做 301 轉址）**：`www.tfoodies.com`（**主網域/canonical**）、
...
>
> **切換步驟（已完成，留存紀錄）**：① DNS：www 設 CNAME、apex 設 A（靜態 IP），各域加 asuid TXT →
> ② 對每個 hostname 跑下方步驟 1 的 `hostname add`/`bind` 完成驗證並各自產生受管憑證 →
> ③ 用 `certificate list` 取得每張憑證名 → ④ 把 `storeCustomDomains` 清單填成 4 項正式網域（主網域擺第 1 項）→
> ⑤ commit + push `infra/**`，部署後即冪等固定。
>
> ⚠️ **實際踩過的坑**：④ 這步一開始漏做——網域改用 CLI 手動 `hostname bind` 上去，`storeCustomDomains`
> 清單卻沒同步更新。2026-06-30 一次無關的 `infra/**` push（改運費金額）觸發 infra 整包部署，把
> `tfoodies.com`/`tfoodies.com.tw` 洗掉，隔天才發現並手動重綁 + 補寫回清單。**教訓：CLI 手動 bind 之後，
> 必須在同一個 PR 就把清單改掉，不要分開兩次做。**
...
> `ingress.customDomains` 以迴圈展開），目前清單為 5 項：`www.tfoodies.com`（主網域）、`tfoodies.com`、
> `www.tfoodies.com.tw`、`tfoodies.com.tw`、`tfoodies-store.4webdemo.com`（過渡網域，保留），各自引用其
> 受管憑證（環境 `tfoodies-cae-dev`）。**再換/加網域時改 `storeCustomDomains` 清單預設值**，不要只在 CLI
> 手動綁。每個新網域第一次仍需先做下方步驟 1~2 完成 DNS 驗證、產生受管憑證，再把名稱填回清單。
...
| **infra 部署報 `ContainerAppEnvironmentMismatch`（`tfoodies-store already exists in a different environment`）** | ⚠️ **env 命名陷阱**：bicep 的 managed environment 名為 `tfoodies-cae-${env}`（帶 env），但 `tfoodies-store`、`tfoodies-api` 名稱**固定不帶 env**。線上 store 實際跑在 **`tfoodies-cae-dev`**，「prod」環境從未真正啟用。用 `env=prod` 部 infra 會想新建 `tfoodies-cae-prod` 並搬移 store，而 Container App **不能跨環境搬移**→失敗。**正確做法：infra 一律用 `env=dev` 部署**（Function App `tfoodies-api` 名稱不帶 env，dev 部署即會更新線上所有 App Settings，如運費 `Order__FreightAmount`）。除非要正式啟用 prod 環境（須先刪線上 store 再於 prod 重建，會中斷服務），否則勿選 prod。 |
