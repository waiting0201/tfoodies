...
- **訪客結帳的預設密碼曾是明文手機號（2026-07-29 已修）**：`ResolveGuestMemberAsync` 在建立新會員時，若請求沒帶密碼就把 `BuyerMobile` **明文**寫進 `Members.password`。手機號在訂單、收件人資訊、客服對話裡到處都是，等於任何拿到號碼的人都能登入該帳號。已改為寫入一組隨機 24 bytes 的 PBKDF2 雜湊（＝「尚未設定密碼」，無人能登入），顧客要登入一律走 `POST /auth/forgot-password`。⚠️ 切勿恢復「手機號當預設密碼」。
