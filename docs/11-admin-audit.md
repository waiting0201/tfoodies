...
- 商品排序（2026-07-27）：清單頁「排序」欄 inline 數字輸入，`@change` 送 `PUT /admin/products/sort`（body `[{id, sort}]`）即時存檔；存檔後不自動重載，改以提示列「套用新順序」手動重載，避免連續調整時該列跳走。商品表單頁亦保留單筆排序欄位
- 品牌清單（唯讀）
- 商品分類清單（唯讀）
...
- ~~商品排序（`SortProducts`，手動 `sort` 欄位）~~ ✅ 2026-07-27 完成（見上方「已完成」）
