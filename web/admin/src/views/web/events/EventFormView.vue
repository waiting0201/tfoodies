...
const form = reactive<EventDetail>({
  title: '',
  summary: '',
  photo: '',
  intro: '',
...
onMounted(async () => {
  ...
  loading.value = true
  try {
    const data = await apiFetch<EventDetail>(`/admin/cms/events/${id}`)
    if (data.eventDate) data.eventDate = data.eventDate.slice(0, 10)
...
async function save() {
  ...
  try {
    if (isEdit.value) {
      await apiFetch(`/admin/cms/events/${id}`, { method: 'PUT', body: JSON.stringify(body) })
    } else {
      await apiFetch('/admin/cms/events', { method: 'POST', body: JSON.stringify(body) })
    }
...
<template>
  <main class="event-form">
    <div class="event-form__header">
      ...
      <h1 class="event-form__title">{{ isEdit ? '編輯' : '新增' }}活動</h1>
    </div>
    ...
    <form v-else class="form-card" @submit.prevent="save">
      ...
      <div class="form-actions">
        <button type="button" class="btn btn--ghost" @click="router.push('/admin/web/events')">取消</button>
...
<style scoped>
.event-form {}
.event-form__header { display:flex; align-items:center; gap:1rem; margin-bottom:1.5rem; flex-wrap:wrap; }
.event-form__title { margin:0; font-family:var(--tf-font-heading,inherit); color:var(--tf-color-primary-dark); font-size:1.5rem; }
