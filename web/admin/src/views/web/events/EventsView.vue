...
async function load() {
  ...
  error.value = ''
  try {
    const data = await apiFetch<PagedResult>(`/admin/cms/events?page=${page.value}&pageSize=${pageSize}`)
...
async function deleteEvent(id: string, title: string) {
  ...
  try {
    await apiFetch(`/admin/cms/events/${id}`, { method: 'DELETE' })
    await load()
  } catch (e: any) {
...
<template>
  <main class="events">
    <div class="events__header">
      <h1 class="events__title">活動管理</h1>
      <button class="btn btn--primary" @click="router.push('/admin/web/events/new')">
        + 新增
      </button>
    </div>

    <p v-if="loading" class="events__muted">載入中…</p>
    <p v-else-if="error" class="events__error">{{ error }}</p>
    ...
    <template v-else>
        ...
        <table class="data-table">
          <thead>
            <tr>
              ...
              <th>標題</th>
              <th style="width:110px">活動日期</th>
              <th style="width:80px">活動圖</th>
              <th class="action-th">操作</th>
            </tr>
          </thead>
          <tbody>
            <tr v-if="items.length === 0">
              <td colspan="6" class="empty-cell">目前沒有活動</td>
            </tr>
            <tr v-for="ev in items" :key="ev.eventId" class="data-table__row">
              <td>{{ ev.sort }}</td>
              <td>
                <img v-if="ev.photo" :src="toBlobUrl(ev.photo)" :alt="ev.title" class="thumb" />
              ...
              <td>
                <button
                  ...
                  @click="router.push(`/admin/web/events/${ev.eventId}/photos`)"
                >活動圖</button>
              </td>
              <td>
                <div class="action-cell">
                  <button
                    class="btn btn--ghost btn--sm"
                    @click="router.push(`/admin/web/events/${ev.eventId}/edit`)"
                  >編輯</button>
                  <button
                    class="btn btn--danger-ghost btn--sm"
                    @click="deleteEvent(ev.eventId, ev.title)"
                  >刪除</button>
                </div>
              </td>
            </tr>
      ...
      <div class="events__pagination">
        ...
        <span class="events__page-info">第 {{ page }} 頁（共 {{ total }} 筆）</span>
...
<style scoped>
.events {}
.events__header { display:flex; align-items:center; justify-content:space-between; margin-bottom:1.25rem; }
.events__title { font-family:var(--tf-font-heading); color:var(--tf-color-primary-dark); margin:0; }
.events__error { color:#dc3545; }
.events__muted { color:var(--tf-color-muted); }
...
.events__pagination { display:flex; align-items:center; gap:0.75rem; justify-content:flex-end; margin-top:1rem; }
.events__page-info { font-size:0.875rem; color:var(--tf-color-muted); }
