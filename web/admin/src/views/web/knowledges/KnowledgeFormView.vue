...
const form = reactive<KnowledgeDetail>({
  question: '',
  photo: '',
  answer: '',
  keyword: '',
...
onMounted(async () => {
  ...
  loading.value = true
  try {
    const data = await apiFetch<KnowledgeDetail>(`/admin/cms/knowledges/${id}`)
...
async function save() {
  ...
  try {
    if (isEdit.value) {
      await apiFetch(`/admin/cms/knowledges/${id}`, { method: 'PUT', body: JSON.stringify(body) })
    } else {
      await apiFetch('/admin/cms/knowledges', { method: 'POST', body: JSON.stringify(body) })
    }
    router.push('/admin/web/knowledges')
  } catch (e: any) {
...
<template>
  <main class="knowledge-form">
    <div class="knowledge-form__header">
      <button class="btn btn--ghost btn--sm" @click="router.push('/admin/web/knowledges')">
        &larr; 返回食知識清單
      </button>
      <h1 class="knowledge-form__title">{{ isEdit ? '編輯' : '新增' }}食知識</h1>
    </div>
      ...
      <div class="form-section">
        ...
        <div class="form-row">
          <div class="form-field form-field--grow">
            ...
            <textarea v-model="form.answer" class="input input--textarea" rows="8"></textarea>
          </div>
        </div>
      </div>
      ...
      <div class="form-section">
        ...
        <div class="form-row">
          <div class="form-field form-field--grow">
            ...
            <textarea v-model="form.description" class="input input--textarea" rows="3"></textarea>
          </div>
        </div>
      </div>

      <div class="form-actions">
        <button type="button" class="btn btn--ghost" @click="router.push('/admin/web/knowledges')">取消</button>
...
<style scoped>
.knowledge-form {}
.knowledge-form__header { display:flex; align-items:center; gap:1rem; margin-bottom:1.5rem; flex-wrap:wrap; }
.knowledge-form__title { margin:0; font-family:var(--tf-font-heading,inherit); color:var(--tf-color-primary-dark); font-size:1.5rem; }
