...
async function load() {
  ...
  error.value = ''
  try {
    const data = await apiFetch<PagedResult>(`/admin/cms/issues?page=${page.value}&pageSize=${pageSize}`)
...
async function deleteIssue(id: string, title: string) {
  if (!confirm(`確定要刪除「${title}」嗎？`)) return
  try {
    await apiFetch(`/admin/cms/issues/${id}`, { method: 'DELETE' })
    await load()
  } catch (e: any) {
...
<template>
  <main class="issues">
    <div class="issues__header">
      <h1 class="issues__title">特集管理</h1>
      <button class="btn btn--primary" @click="router.push('/admin/web/issues/new')">
        + 新增
      </button>
    </div>

    <p v-if="loading" class="issues__muted">載入中…</p>
    <p v-else-if="error" class="issues__error">{{ error }}</p>
    ...
    <template v-else>
        ...
        <table class="data-table">
          ...
          <tbody>
            <tr v-if="items.length === 0">
              <td colspan="5" class="empty-cell">目前沒有特集</td>
            </tr>
            <tr v-for="item in items" :key="item.issueId" class="data-table__row">
              <td>{{ item.sort }}</td>
              <td>
                <img v-if="item.photo" :src="toBlobUrl(item.photo)" :alt="item.title" class="thumb" />
                <span v-else class="text-muted">—</span>
              </td>
              <td>{{ item.title }}</td>
              <td>
                <span :class="['badge', item.ispublish ? 'badge--on' : 'badge--off']">
                  {{ item.ispublish ? '上架' : '下架' }}
                </span>
              </td>
              <td>
                <div class="action-cell">
                  <button
                    ...
                    @click="router.push(`/admin/web/issues/${item.issueId}/edit`)"
                  >編輯</button>
                  <button
                    class="btn btn--danger-ghost btn--sm"
                    @click="deleteIssue(item.issueId, item.title)"
                  >刪除</button>
                </div>
              </td>
            </tr>
      ...
      <div class="issues__pagination">
        ...
        <span class="issues__page-info">第 {{ page }} 頁（共 {{ total }} 筆）</span>
...
<style scoped>
.issues {}
.issues__header { display:flex; align-items:center; justify-content:space-between; margin-bottom:1.25rem; }
.issues__title { font-family:var(--tf-font-heading); color:var(--tf-color-primary-dark); margin:0; }
.issues__error { color:#dc3545; }
.issues__muted { color:var(--tf-color-muted); }
...
.issues__pagination { display:flex; align-items:center; gap:0.75rem; justify-content:flex-end; margin-top:1rem; }
.issues__page-info { font-size:0.875rem; color:var(--tf-color-muted); }
