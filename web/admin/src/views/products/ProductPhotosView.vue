...
async function loadProduct() {
  try {
    const d = await apiFetch<{ product: { title: string } }>(`/admin/products/${productId}`)
    productTitle.value = d.product?.title ?? ''
  } catch { /* 無法取得標題時不中斷 */ }
}
...
async function loadPhotos() {
  ...
  error.value = ''
  try {
    photos.value = await apiFetch<ProductPhoto[]>(`/admin/products/${productId}/photos`)
  } catch (e) {
...
async function addPhoto() {
  ...
  addError.value = ''
  try {
    await apiFetch(`/admin/products/${productId}/photos`, {
      method: 'POST',
...
async function updateSort(p: ProductPhoto) {
  try {
    await apiFetch(`/admin/products/${productId}/photos/${p.productphotoId}`, {
      method: 'PUT',
...
async function deletePhoto(photoId: string) {
  ...
  try {
    await apiFetch(`/admin/products/${productId}/photos/${photoId}`, { method: 'DELETE' })
    photos.value = photos.value.filter(p => p.productphotoId !== photoId)
  } catch (e) {
...
<template>
  <main class="photos">
    <div class="photos__header">
      <button class="btn btn--ghost btn--sm" @click="router.push('/admin/products')">&larr; 返回商品清單</button>
      <h1 class="photos__title">
        商品圖庫管理
        <span v-if="productTitle" class="photos__subtitle">— {{ productTitle }}</span>
      </h1>
    </div>
    ...
    <div v-else class="card">
      <table class="data-table">
        ...
        <tbody>
          ...
          <tr v-for="p in photos" :key="p.productphotoId" class="data-table__row">
            <td>
              <input v-model.number="p.sort" class="input input--sort" type="number" min="0" @change="updateSort(p)" />
            </td>
            <td>
              <img v-if="p.photo" :src="toBlobUrl(p.photo)" alt="商品圖片" class="thumb-lg" />
            </td>
            <td class="url-cell">{{ p.photo }}</td>
            <td>
              <div class="action-cell">
                <button class="btn btn--danger-ghost btn--sm" @click="deletePhoto(p.productphotoId)">刪除</button>
              </div>
            </td>
          </tr>
        </tbody>
