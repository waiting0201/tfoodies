...
const panelTarget = ref<Tag | null>(null)
...
async function load() {
  ...
  error.value = ''
  try {
    items.value = await apiFetch<Tag[]>('/admin/tags')
  } catch (e) {
...
async function save() {
  ...
  try {
    if (panelMode.value === 'create') {
      await apiFetch('/admin/tags', { method: 'POST', body: JSON.stringify({ title: form.title }) })
    } else {
      await apiFetch(`/admin/tags/${panelTarget.value!.tagid}`, { method: 'PUT', body: JSON.stringify({ title: form.title }) })
    }
    closePanel()
    await load()
  } catch (e) {
...
async function confirmDelete() {
  ...
  deleteError.value = ''
  try {
    await apiFetch(`/admin/tags/${deleteTarget.value.tagid}`, { method: 'DELETE' })
...
<template>
  <main class="tags">
    <div class="tags__header">
      <h1 class="tags__title">標籤管理</h1>
      <button class="btn btn--primary" @click="openCreate">+ 新增標籤</button>
    </div>

    <p v-if="loading" class="tags__muted">載入中…</p>
    <p v-if="error" class="tags__error">{{ error }}</p>
    ...
    <div v-if="!loading" class="card">
      <table class="data-table">
        ...
        <tbody>
          <tr v-for="t in items" :key="t.tagid" class="data-table__row">
          ...
          <tr v-if="items.length === 0">
            <td colspan="2" class="empty-cell">目前沒有標籤資料</td>
          </tr>
        </tbody>
      </table>
    </div>
    ...
    <div v-if="panelMode" class="panel-overlay" @click.self="closePanel">
      <aside class="side-panel">
        <div class="panel__header">
          <h2 class="panel__title">{{ panelMode === 'create' ? '新增標籤' : '編輯標籤' }}</h2>
        ...
        <div class="panel__body">
          <div class="form-field">
            <label class="form-field__label">標籤名稱 <span class="required">*</span></label>
            <input v-model="form.title" class="form-field__input" placeholder="例：有機、非基改" />
          </div>
    ...
    <div v-if="deleteTarget" class="modal-overlay" @click.self="deleteTarget = null">
      <div class="modal">
        <div class="modal__header">
          <h3 class="modal__title">確認刪除標籤</h3>
        </div>
        <div class="modal__body">
          <p>確定要刪除標籤 <strong>{{ deleteTarget.title }}</strong> 嗎？</p>
          <p class="modal__hint">若已有商品使用此標籤，系統將拒絕刪除。</p>
...
<style scoped>
.tags {}
.tags__header { display: flex; align-items: center; justify-content: space-between; margin-bottom: 1.25rem; }
.tags__title { font-family: var(--tf-font-heading); color: var(--tf-color-primary-dark); margin: 0; }
.tags__error { color: #dc3545; }
.tags__muted { color: var(--tf-color-muted); }
