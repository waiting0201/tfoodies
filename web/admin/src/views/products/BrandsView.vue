...
const panelTarget = ref<Brand | null>(null)
const form = reactive({ title: '', subtitle: '', sort: 0, isDisplay: true })
...
function openCreate() {
  form.title = ''
  form.subtitle = ''
  form.sort = items.value.length > 0 ? Math.max(...items.value.map(b => b.sort)) + 1 : 1
  form.isDisplay = true
  saveError.value = ''
...
async function save() {
  ...
  saveError.value = ''
  try {
    const payload = { title: form.title, subtitle: form.subtitle, sort: form.sort, isDisplay: form.isDisplay }
    if (panelMode.value === 'create') {
      await apiFetch('/admin/brands', { method: 'POST', body: JSON.stringify(payload) })
    } else {
      await apiFetch(`/admin/brands/${panelTarget.value!.brandid}`, { method: 'PUT', body: JSON.stringify(payload) })
    }
    closePanel()
    await load()
  } catch (e) {
...
<template>
  <main class="brands">
    <div class="brands__header">
      ...
      <button class="btn btn--primary" @click="openCreate">+ 新增品牌</button>
    </div>
        ...
        <tbody>
          <tr v-for="b in items" :key="b.brandid" class="data-table__row">
            ...
            <td class="action-cell">
              <button class="btn btn--sm btn--ghost" @click="openEdit(b)">編輯</button>
              <button class="btn btn--sm btn--danger-ghost" @click="askDelete(b)">刪除</button>
            </td>
          </tr>
          <tr v-if="items.length === 0">
            <td colspan="5" class="empty-cell">目前沒有品牌資料</td>
          </tr>
        </tbody>
      </table>
    </div>
    ...
    <div v-if="panelMode" class="panel-overlay" @click.self="closePanel">
      <aside class="side-panel">
        <div class="panel__header">
          <h2 class="panel__title">{{ panelMode === 'create' ? '新增品牌' : '編輯品牌' }}</h2>
        ...
        <div class="panel__body">
          <div class="form-field">
            <label class="form-field__label">品牌名稱 <span class="required">*</span></label>
            <input v-model="form.title" class="form-field__input" placeholder="品牌名稱" />
          </div>
          <div class="form-field">
            <label class="form-field__label">副標題</label>
            <input v-model="form.subtitle" class="form-field__input" placeholder="副標題（選填）" />
          </div>
          <div class="form-row">
            <div class="form-field">
            ...
            <div class="form-field">
              <label class="form-field__label">顯示於前台</label>
              <label class="checkbox-option">
                <input v-model="form.isDisplay" type="checkbox" />
                <span>顯示</span>
              </label>
            </div>
          </div>
